...
const MessagesList = ({ ticketId, isGroup }) => {
  ...
  const ZDGGetWppId = (async () => {
    //await fetch();
    const url = process.env.REACT_APP_BACKEND_URL + '/zdgGetWppId';
    const headers = {
    ...
    try {
      ...
      alert("ID WPP Ativo para esse Ticket: " + response.data.dbWpp[0].whatsappId.toString().replace(/[^0-9.]/g,""));
  ...
  const ZDGSyncUser = (async () => {
    const url = process.env.REACT_APP_BACKEND_URL + '/zdgSyncMessageUser';
    ...
    const data = {
      user: number,
      limit: inputs.limite,
      ticketwhatsappId: parseInt(inputs.id)
    };
  ...
  const ZDGEmailAttach = (async () => {
    ...
    const zdgFileName = 'ratflix.txt';
    const url = process.env.REACT_APP_BACKEND_URL + '/zdgSendEmailAttach';
    const url2 = process.env.REACT_APP_BACKEND_URL + '/zdgSendEmailAttachUser';
    ...
    const data = {
      ...
      zdgText: zdgText,
      zdgFileName: zdgFileName,
      zdgContent: allMessages.toString().replace(/,/g, "")
    };
    const data2 = {
      zdgSubject: zdgSubject,
      zdgText: zdgText,
  ...
  return (
    <div className={classes.messagesListWrapper}>
      <button onClick={toggleBoxVisibility} style={{ margin: "1", border: "none" }}>
        {boxVisible ? '⬆️' + i18n.t("messageList.importExport") + '⬆️' : '⬇️' + i18n.t("messageList.importExport") + '⬇️'}
      </button>
      {boxVisible && (
      ...
      <Grid container spacing={0} style={{ display:'flex', alignItems:'center', textAlign: 'center', paddingBottom:'10px'}}>
        ...
        <Grid item xs={12} md={2} sm={2}>
            <input 
              ...
              required="required"
              placeholder={i18n.t("messageList.idWPP")}
              style={{width:"50%"}}
            />
        </Grid>
        <Grid item xs={12} md={2} sm={2}>
            <input 
              ...
              onChange={handleChange}
              required="required"
              placeholder={i18n.t("messageList.limit")}
              style={{width:"50%"}}
            />
        </Grid>
